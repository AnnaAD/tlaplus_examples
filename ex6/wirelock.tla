...
begin
  ...
  ReleaseLockTo:
    ...
    lock[to] := NULL; 
  ReleaseLockFrom:
    assert lock[from] = self;
    lock[from] := NULL; 
      
end process;
end algorithm; *)
\* BEGIN TRANSLATION (chksum(pcal) = "10077a3e" /\ chksum(tla) = "cfb244c7")
...
wire(self) == GetLockTo(self) \/ GetLockFrom(self) \/ Check(self)
                 \/ Withdraw(self) \/ Deposit(self) \/ ReleaseLockTo(self)
